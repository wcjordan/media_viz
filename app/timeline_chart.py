...
def create_timeline_chart(weeks_df: pd.DataFrame, bars_df: pd.DataFrame) -> go.Figure:
    ...
    for week_idx, group in grouped_bars:
        ...
        for i, (_, bar) in enumerate(group.iterrows()):
            ...
            x_offset = bar["entry_id"] * 0.2

            # Add bar
            rgb_tuple = tuple(
                int(bar["color"].lstrip("#")[i : (i + 2)], 16) for i in (0, 2, 4)
            )
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    line=dict(
                        color=f"rgba{rgb_tuple + (bar['opacity'],)}",
                        width=10,
                    ),
                    name=bar["title"],
                    text=f"{bar['title']} ({bar['type']})<br>"
                    f"Start: {bar['start_date']}<br>"
                    f"Finish: {bar['end_date']}<br>"
