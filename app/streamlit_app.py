...
BAR_HEIGHT = 0.8  # Height of each bar (0-1)
...
def compute_week_index(date_str: str, min_date: datetime) -> int:
    """
    ...
    Returns:
        Week index (integer)
    """
    date = parse_date(date_str).replace(tzinfo=None)
    delta = date - min_date
    return delta.days // 7


def get_date_range(entries: List[Dict]) -> Tuple[datetime, datetime]:
    """
    ...
    for entry in entries:
        if "start_date" in entry and entry["start_date"]:
            all_dates.append(parse_date(entry["start_date"]).replace(tzinfo=None))
        if "finish_date" in entry and entry["finish_date"]:
            all_dates.append(parse_date(entry["finish_date"]).replace(tzinfo=None))
...
def calculate_opacity(start_week: int, end_week: int, current_week: int, 
                      has_start: bool, has_end: bool, long_duration: bool) -> float:
    """
...
def prepare_timeline_data(entries: List[Dict]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    min_date, max_date = get_date_range(entries)
    ...
    for entry_idx, entry in enumerate(entries):
        ...
        has_start = "start_date" in entry and entry["start_date"]
        has_end = "finish_date" in entry and entry["finish_date"]
        ...
        start_week = compute_week_index(entry["start_date"], min_date) if has_start else None
        end_week = compute_week_index(entry["finish_date"], min_date) if has_end else None
        ...
        duration_weeks = end_week - start_week if start_week is not None and end_week is not None else 0
        long_duration = duration_weeks > (LONG_DURATION_MONTHS * 4)  # Approx. 4 weeks per month
        ...
        for week in range(start_week, end_week + 1):
            ...
            if opacity > 0:
                bars.append({
                    ...
                    "canonical_title": entry.get("canonical_title", entry.get("title", "Unknown")),
                    "type": media_type,
                    "week_index": week,
                    "color": color,
                    "opacity": opacity,
...
def create_timeline_chart(weeks_df: pd.DataFrame, bars_df: pd.DataFrame) -> go.Figure:
    ...
    for i, year in enumerate(years):
        ...
        # Add rectangle for year background (alternating shades)
        color = "rgba(50, 50, 50, 0.2)" if i % 2 == 0 else "rgba(70, 70, 70, 0.2)"
        fig.add_shape(
            type="rect",
            x0=-0.5,
            x1=max_week + 0.5,
...
def main():
    ...
    if media_entries:
        ...
        with tab1:
            ...
            if not weeks_df.empty and not bars_df.empty:
                # Create and display timeline chart
                fig = create_timeline_chart(weeks_df, bars_df)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Could not create timeline visualization. Check data format.")
