...
def test_parse_date_range_cross_year(caplog):
    ...
    assert "Error parsing end date 'Jan 3': Rows are not expected to cross years" in caplog.text
...
def test_parse_date_range_no_start_month(caplog):
    ...
    assert "Error parsing start date '15': Month not found in start date string" in caplog.text
    assert start_date == None
    assert end_date == None
    assert current_year == 2023
...
def test_year_propagation():
    ...
    csv_content = "\n".join([line.strip() for line in csv_content.getvalue().splitlines()])
